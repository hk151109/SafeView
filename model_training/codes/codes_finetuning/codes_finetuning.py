...
TRAIN_DIR = '/mnt/d/hari/LSPD/train/'
VALID_DIR = '/mnt/d/hari/LSPD/val/'
TEST_DIR = '/mnt/d/hari/LSPD/test/'
...
def fine_tune_model(model, base_model, train_ds, val_ds, class_weights):
    ...
    # Typically the last convolutional block in MobileNetV3Small
    for layer in base_model.layers[-30:]:  # Adjust the number based on how many layers you want to unfreeze
    ...
    checkpoint = ModelCheckpoint(
        'mobilenetv3_nsfw_model_finetuned_best.h5',
        monitor='val_accuracy',
        save_best_only=True,
        verbose=1
    )
    
    # Fine-tune for fewer epochs with the lower learning rate
...
def compare_models(base_evaluation, finetuned_evaluation):
    ...
    # Optionally create a comparison visualization
    plt.figure(figsize=(12, 6))
    
    # Plot confusion matrix differences
    diff_cm = finetuned_evaluation['confusion_matrix'] - base_evaluation['confusion_matrix']
    plt.subplot(1, 2, 1)
    sns.heatmap(diff_cm, annot=True, fmt='d', cmap='coolwarm', xticklabels=CLASSES, yticklabels=CLASSES)
    plt.title('Difference in Confusion Matrix\n(Positive = Fine-tuned better)')
    
    # Plot per-class accuracy differences
    base_acc = np.diag(base_evaluation['confusion_matrix']) / np.sum(base_evaluation['confusion_matrix'], axis=1)
    ft_acc = np.diag(finetuned_evaluation['confusion_matrix']) / np.sum(finetuned_evaluation['confusion_matrix'], axis=1)
...
def main():
    ...
    print("Step 5: Fine-tuning the model with unfrozen layers...")
